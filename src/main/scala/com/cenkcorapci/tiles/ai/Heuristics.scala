...
object Heuristics {
  ...
  implicit class BestByMinimax(state: GameState) {
    def getBestMoveWithMinimax(searchDepth: Int): Option[GameState] = {
      def minimax(states: Seq[GameState],
                  depth: Int = searchDepth,
                  max: Boolean = true): Option[GameState] = {
        if (depth == 0) {
          ...
          if (max)
            states
              .map(_.getNextStatesForPlayer1)
              .flatMap(s => minimax(s, depth - 1, !max))
              .sortBy(-_.stateScore)
          else
            states
              .map(_.getNextStatesForPlayer2)
              .flatMap(s => minimax(s, depth - 1, !max))
              .sortBy(_.stateScore)
    ...
    def getBestMoveWithMinimaxWithAlphaBetaPruning(
        searchDepth: Int): Option[GameState] =
